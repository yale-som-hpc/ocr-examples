...
def read_catalog(path: Path) -> list[dict[str, str]]:
    with path.open(newline="", encoding="utf-8") as handle:
        rows = [row for row in csv.DictReader(handle, delimiter="\t") if row.get("filename")]
    if not rows:
...
def download(url: str, path: Path) -> None:
    request = urllib.request.Request(url, headers={"User-Agent": "yale-som-hpc-ocr-examples"})
    with urllib.request.urlopen(request, timeout=60) as response:
        data = response.read()
    if not data.startswith(b"%PDF"):
        print(f"ERROR: downloaded file does not look like a PDF: {url}", file=sys.stderr)
        sys.exit(1)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--manifest", type=Path, default=Path("data/samples/manifest.txt"))
