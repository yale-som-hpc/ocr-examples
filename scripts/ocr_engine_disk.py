...
def write_sidecar(input_pdf: Path, output: Path, engine: str, started: float, params: dict) -> None:
...
def run_docling(input_pdf: Path, outdir: Path) -> Path:
    ...
    output.write_text(candidates[-1].read_text(encoding="utf-8", errors="replace"), encoding="utf-8")
    write_sidecar(input_pdf, output, "docling", started, {"command": cmd, "raw_output_dir": str(run_dir)})
    return output
...
def run_mlx_vlm(input_pdf: Path, outdir: Path, engine: str, model_id: str, scale: float, max_tokens: int) -> Path:
    ...
    for page_num, image in render_pages(input_pdf, scale):
        ...
        result = generate(model, processor, prompt, image=[image], max_tokens=max_tokens, verbose=False)
        text = result.text if hasattr(result, "text") else str(result)
        page_texts.append(f"<!-- page {page_num} -->\n{text.strip()}\n")
    output = out_path(input_pdf, outdir, engine)
    output.write_text("\n\n".join(page_texts), encoding="utf-8")
    write_sidecar(input_pdf, output, engine, started, {"model": model_id, "scale": scale, "max_tokens": max_tokens})
    return output
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--engine", choices=("pypdf", "docling", "olmocr2", "deepseek_ocr", "glm_ocr", "unlimited_ocr"), required=True)
    parser.add_argument("--outdir", type=Path, default=Path("results/engine-disk"))
    parser.add_argument("--mlx-model", help="Override MLX model id for olmocr2/deepseek_ocr/glm_ocr")
...
def main() -> None:
    ...
    if args.engine == "pypdf":
        ...
        raise SystemExit("unlimited_ocr has no local/disk backend; use the SGLang HPC tunnel path")
    print(output)
